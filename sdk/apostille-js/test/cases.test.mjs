...
// web/apostille-cases.test.mjs. This exercises the BUILT package, so the
// runner is duplicated here rather than imported from the web test.
const file = JSON.parse(await readFile(new URL("../spec/vector-cases.json", import.meta.url), "utf8"));
...
test("case file guards", () => {
    assert.equal(file.protocol, core.PROTOCOL);
...
test("issuer_cases", () => {
    ...
    for (const c of file.issuer_cases) {
        try {
            assert.equal(core.validIssuer(c.value), c.expect === "accept", JSON.stringify(c.value));
        } catch (err) {
